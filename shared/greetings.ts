import { dateKey, dayStart } from './domain';
...
export const HolidayGreetings: Record<string, string> = {
...
export function dailyGreeting(now = Date.now()) {
  ...
  return {
    season,
    quote:
      HolidayGreetings[key] ||
      HolidayGreetings[key.slice(5)] ||
      lines[season][Math.floor(dayStart(now) / DAY_MS) % lines[season].length],
